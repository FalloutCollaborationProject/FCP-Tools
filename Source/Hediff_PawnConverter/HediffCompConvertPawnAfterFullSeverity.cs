...
namespace Hediff_PawnConverter
{
    ...
    {
        public HediffCompProperties_ConvertPawnAfterFullSeverity Props => (HediffCompProperties_ConvertPawnAfterFullSeverity)props;
                ...
                if(Props.isUsingImmunityList)
                {
                    if (ModsConfig.BiotechActive && !Props.immuneXenotypeDef.NullOrEmpty())
                    {
                        if (!Props.immuneXenotypeDef.Contains(parent.pawn.genes.Xenotype))
                        {
                            ...
                            {
                                DoTransformation(Props.pawnKindDefs.RandomElement(), Find.FactionManager.FirstFactionOfDef(Props.factionDef));
                            }
                        }
                        else
                        {

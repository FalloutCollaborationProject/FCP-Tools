        ...
        public static void ShouldDoNow_GoodWillCheck(ref bool __result, Bill_Production __instance)
        {
            ...
            try
            {
                ...
                if (targetFact == null)
                {
                    ...
                    {
                        Messages.Message("Goodwill_FactionDontExist".Translate(targetFact.Name, modExtension.minimumGoodwill), MessageTypeDefOf.NegativeEvent);
                    }
                    __instance.suspended = true;
                    __result = false;
                }
                else
                {
                    if (targetFact.defeated)
                    {
                        if (modExtension.uncraftableIfFactionDefeated)
                        {
                            if (!__instance.suspended)
                            {
                                Messages.Message("Goodwill_FactionDefeated".Translate(targetFact.Name, modExtension.minimumGoodwill), MessageTypeDefOf.NegativeEvent);
                            }
                            __instance.suspended = true;
                            __result = false;
                        }
            ...
            {
                Log.Error($"[RecipeGoodwillLimiter] Error: {ex}");
            }
        }
    }
}

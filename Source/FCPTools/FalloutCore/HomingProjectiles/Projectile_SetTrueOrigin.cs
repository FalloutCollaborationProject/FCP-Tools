...
public static class Projectile_SetTrueOrigin
{
    ...
    public static MethodInfo InterceptChanceFactorFromDistanceInfo =
        AccessTools.Method(typeof(VerbUtility), "InterceptChanceFactorFromDistance");

    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> codeInstructions)
    {
        var codes = codeInstructions.ToList();
        var field = AccessTools.Field(typeof(Projectile), "origin");
    ...
    public static Vector3 GetTrueOrigin(Projectile projectile)
    {
        ...
        return Traverse.Create(projectile).Field("origin").GetValue<Vector3>();
    }
}

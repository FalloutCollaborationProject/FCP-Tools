...
public class JobGiver_ReactToDistantThreat : ThinkNode_JobGiver
{
    ...
    {
        if (!IsValidPawnForReaction(pawn)) return null;
        if (IsCurrentlyReacting(pawn)) return null;
        Pawn attacker = FindAttacker(pawn);
            
        if (attacker == null || attacker.IsPsychologicallyInvisible() || PawnUtility.PlayerForcedJobNowOrSoon(pawn)) return null;
        Ability ability = GetRandomAbility(pawn);
        return ability?.verb == null ? null : CreateAbilityJob(attacker, ability);
    }
        
    private static bool IsValidPawnForReaction(Pawn pawn)
    {
        return !pawn.Downed && !pawn.Dead && pawn.RaceProps.Animal && pawn.abilities != null;
    }
    ...
    {
        Job job = JobMaker.MakeJob(FCPDefOf.FCP_AICastAbilityAtTarget, attacker);

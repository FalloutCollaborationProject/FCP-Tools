...
namespace FCP.Core
{
    public class JobDriver_AICastAbilityAtTarget : JobDriver
    {
        ...
        {
            if (job.ability == null || job.verbToUse == null) yield break;
        ...
        private Toil CreateMoveToTargetToil()
        {
            return new Toil
            {
                initAction = () =>
                {
                    ...
                    {
                        FCPLog.Warning("No valid position found within the range.");
        ...
        {
            Toil castAbility = Toils_Combat.CastVerb(TargetIndex.A, TargetIndex.B, canHitNonTargetPawns: false);
            if (job.ability != null && job.ability.def.showCastingProgressBar && job.verbToUse != null)
            {
                castAbility.WithProgressBar(TargetIndex.A, () => job.verbToUse.WarmupProgress);
            }
            return castAbility;
        }
        ...
        private IntVec3 GetValidPositionNearTarget(IntVec3 targetPos, float maxRange, float minRange)
        {
            for (int i = 0; i < 30; i++)
            {
                IntVec3 randomPos = targetPos + GenRadial.RadialPattern[Rand.Range(0, GenRadial.NumCellsInRadius(maxRange))];
                float distanceToTarget = (randomPos - targetPos).LengthHorizontal;

                if (distanceToTarget <= maxRange && distanceToTarget >= minRange && randomPos.InBounds(pawn.Map) && randomPos.Standable(pawn.Map))
                {
                    return randomPos;
                }
            }

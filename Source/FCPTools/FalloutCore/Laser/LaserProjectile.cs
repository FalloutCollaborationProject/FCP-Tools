...
namespace FCP.Core.Laser
{
    ...
    {
        private static readonly PropertyInfo arcHeightFactorProp = AccessTools.Property(typeof(Projectile), "ArcHeightFactor");
        ...
        private void DamageThings()
        {
            ...
            foreach (var thing in targets)
            {
                ...
                if (thing != intendedTarget.Thing &&
                    !Rand.Chance(VerbUtility.InterceptChanceFactorFromDistance(origin, thing.Position)))
                {
                    continue;
                }

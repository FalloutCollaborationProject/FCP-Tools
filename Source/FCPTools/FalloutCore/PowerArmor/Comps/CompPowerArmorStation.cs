...
public class CompPowerArmorStation : ThingComp, IThingHolder, ISearchableContents
{
	...
	public override IEnumerable<FloatMenuOption> CompFloatMenuOptions(Pawn selPawn)
	{
		...
		if (hasApparelToStore)
		{
			...
			{
				selPawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(PowerArmorDefOf.FCP_StorePowerArmor, parent), JobTag.Misc);
			});
		}
		...
		if (cantReason.Accepted || cantReason.Reason.NullOrEmpty() is false)
		{
			if (innerContainer.Any)
			{
				yield return CreateFloatMenuOption("FCP_EquipFromStation".Translate().CapitalizeFirst(), PowerArmorDefOf.FCP_EquipFromStation, cantReason, selPawn);
			}
			if (hasApparelToStore && innerContainer.Any)
			{
				yield return CreateFloatMenuOption("FCP_SwapPowerArmor".Translate().CapitalizeFirst(), PowerArmorDefOf.FCP_SwapPowerArmor, cantReason, selPawn);
			}
		}
	}
